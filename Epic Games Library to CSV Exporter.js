...
// @version      5.7
// @description  Fixes PascalCase only if no spaces exist. Adds Pause/Stop buttons for the export process.
...
(function() {
    ...
    function smartCleanTitle(rawTitle) {
        ...
        // ONLY add spaces if the title currently has NO spaces at all
        if (!cleaned.includes(" ")) {
            cleaned = cleaned.replace(/([a-z0-9])([A-Z])/g, '$1 $2');
        }
        
        // Remove common edition suffixes to help RAWG
    ...
    function createLogger() {
        ...
        titleLabel.innerText = "Exporting Metadata...";
        titleLabel.style.cssText = 'color: #eee; font-size: 11px; font-weight: bold;';
        
        const ctrlGroup = document.createElement('div');
        
        const pauseBtn = document.createElement('button');
        pauseBtn.innerText = "Pause";
        pauseBtn.style.cssText = 'background: #555; color: white; border: none; padding: 2px 8px; border-radius: 3px; font-size: 10px; cursor: pointer; margin-right: 5px;';
        pauseBtn.onclick = () => {
            isPaused = !isPaused;
            pauseBtn.innerText = isPaused ? "Resume" : "Pause";
            pauseBtn.style.background = isPaused ? "#0078f2" : "#555";
        };

        const stopBtn = document.createElement('button');
        stopBtn.innerText = "Stop & Save";
        stopBtn.style.cssText = 'background: #dc3545; color: white; border: none; padding: 2px 8px; border-radius: 3px; font-size: 10px; cursor: pointer;';
        stopBtn.onclick = () => { shouldStop = true; };

        ctrlGroup.appendChild(pauseBtn);
        ctrlGroup.appendChild(stopBtn);
    ...
    async function startExport() {
        ...
        try {
            logMessage("Accessing Epic account history...", "#0078f2");
            ...
            if (skipTags) {
                ...
                logMessage(`Found ${games.length} titles. Starting tag fetch...`, "#0078f2");
                finalData = await fetchTagsWithControls(games, rawgApiKey);
            }

            logMessage("Processing final file...", "#ffc107");
        ...
        setTimeout(() => { 
            ...
            btn.innerText = 'Export Library (v5.7)'; 
            btn.disabled = false; 
    ...
    async function fetchHistory(nextPageToken = '', allGames = []) {
        const url = `https://www.epicgames.com/account/v2/payment/ajaxGetOrder_History?nextPageToken=${nextPageToken}&locale=en-US`;
    ...
    async function fetchTagsWithControls(games, apiKey) {
        ...
        for (let i = 0; i < total; i++) {
            // Check Stop Flag
            if (shouldStop) {
                logMessage("STOPPED: Exporting collected data...", "#dc3545");
                break;
            }
            // Check Pause Flag
            ...
            try {
                const data = await new Promise((res, rej) => {
                    GM_xmlhttpRequest({
                        ...
                        onload: (r) => r.status === 200 ? res(JSON.parse(r.responseText)) : rej(new Error("API Error")),
                        onerror: rej
                    });
                });
                ...
                results.push({ ...games[i], title: displayTitle, tags: "Error" });
            }
